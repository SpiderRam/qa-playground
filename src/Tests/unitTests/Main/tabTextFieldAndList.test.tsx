...
const getDeleteButton = async (textContent: string) => {
	...
	const deleteTarget = listItems.find((item) =>
...
const getListItems = async () => {
	...
	const items = elements
		.map((item, index) => ({
			text: within(item)
				.getByTestId(`mainTabListItem_${index}`)
				.textContent,
			deleteButton: within(item)
				.getByTestId(`mainTabListItemDeleteButton_${index}`)
		}))
	return items
}
...
it('should verify item deletion', async () => {
	...
	if(!listItems) { throw new Error(`No listItems found`)}
